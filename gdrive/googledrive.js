...
function handleSignoutClick(event) {
    ...
    window.location.replace("http://localhost/CAPSTONE/ayawna/google-drive.html");
	}
}
...
function ascending() {
  ...
  // for (var i = 0; i < ASCENDING_ARRAY.length; i++) {
  //   if () {}
  // }
}
...
function buildFiles() {
    ...
    if (DRIVE_FILES.length > 0) {
        for (var i = 0; i < DRIVE_FILES.length; i++) {
          ASCENDING_ARRAY.push(DRIVE_FILES[i].title);
          ...
          // if there's a file extension then its a folder, if wala then its a file
...
// view image
function viewingImage(obj) {
    var image = $(obj).attr("data-id");
    // var src = document.getElementById("viewingTheImage");
    // src.appendChild(image);
    document.getElementById("imageName").value = image;
    document.getElementById("viewingTheImage").src = "http://drive.google.com/uc?export=view&id=" + image;
    $("#transparent-wrapper").show();
    $("#view-box").show();
}
